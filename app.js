...
function recordingUrl(value) {
  ...
  if (/^\/api\/blobs\/[a-f0-9]{32}\.(webm|ogg|m4a|wav|mp3)$/.test(value)) return value;
  if (/^data:audio\/[a-z0-9.+-]+(?:;[a-z0-9.+-]+=[a-z0-9.+-]+)*;base64,[a-z0-9+/=\s]+$/i.test(value)) return value;
  return '';
}
function audioHtml(audio, label) { const text = label ?? t('Voice note'), src = recordingUrl(audio?.url); return src ? `<div class="attached-audio"><strong>🎙 ${escapeHtml(text)}</strong><span>${formatDuration(audio.duration)}</span><audio controls preload="metadata" src="${escapeHtml(src)}" aria-label="${escapeHtml(tf('Play {label}', {label:text}))}"></audio></div>` : ''; }
