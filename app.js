...
const views = {
  ...
  handoff() {
    ...
    return `<div class="handoff-heading"><div><p class="eyebrow">CARE CONTINUITY · TODAY</p><h2>A clear handoff for whoever cares next</h2><p>Built from today's captured observations. Review before sharing or starting care.</p></div><div class="audience-toggle" aria-label="Handoff audience"><button class="${state.handoffAudience==='owner'?'active':''}" data-audience="owner">Owner</button><button class="${state.handoffAudience==='carer'?'active':''}" data-audience="carer">Next carer</button></div></div>
      ...
      <section class="card evidence-panel"><div class="section-head"><div><h2>Evidence for this handoff</h2><p>Open any source observation in ${d.name.split(' ')[0]}'s care timeline.</p></div><button class="ghost" data-go="gallery">View today's gallery</button></div><div class="evidence-list">${summary.observations.length?summary.observations.map(o=>`<button class="evidence-row" data-evidence-dog="${key}" data-evidence-id="${o.id}"><time>${o.time}</time><span><strong>${escapeHtml(o.title)}</strong><small>${escapeHtml(o.text)}</small></span><b>View source →</b></button>`).join(''):'<p class="empty-evidence">No source observations available yet.</p>'}</div></section>`;
  },
