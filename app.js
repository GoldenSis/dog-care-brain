...
function startTranscription() {
  ...
  recognition.onstart=()=>{activeRecognition=recognition;startButton.hidden=true;stopButton.hidden=false;status.textContent=t('Listening… your words will appear in the care update.');};
  ...
  recognition.onend=()=>{if(activeRecognition===recognition)activeRecognition=null;startButton.hidden=false;stopButton.hidden=true;if(!status.textContent || status.textContent===t('Listening… your words will appear in the care update.'))status.textContent=finalText?t('Transcript added. Review and edit it before saving.'):t('Listening stopped. You can try again or type your update.');};
...
function bindView() {
  ...
  if(input){
    ...
    const transcriptionButton=document.querySelector('#start-transcription'), Recognition=window.SpeechRecognition || window.webkitSpeechRecognition;
    document.querySelector('#stop-transcription').onclick=stopTranscription;
    if(Recognition) transcriptionButton.onclick=startTranscription;
    else {transcriptionButton.hidden=true;document.querySelector('#transcription-status').textContent=t('Speech-to-text is not available in this browser. You can still record audio or type your update.');}
