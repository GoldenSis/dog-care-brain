...
function handoffSummary(key) {
  ...
  const highlights=[];
  if(nutrition) highlights.push(`Food and drink: ${nutrition.text}`);
  if(activity) highlights.push(`Movement and enrichment: ${activity.text}`);
  observations.filter(o=>!highlights.some(text=>text.includes(o.text))).slice(0,Math.max(0,3-highlights.length)).forEach(o=>highlights.push(o.text));
  const healthText=health.length
    ? `${health.length} factual health observation${health.length===1?' is':'s are'} linked below. This is not a diagnosis; keep observing and contact a veterinarian if you are concerned.`
    : 'No health-watch observations were recorded today. This reflects today\'s care log, not a clinical assessment.';
  const guidance=state.handoffAudience==='owner'
    ? (health.length ? ['Continue to observe the specific signs noted below.', 'Share any change or concern with the next carer, and contact your veterinarian if concerned.'] : ['Keep the usual home routine for food, rest and movement.', 'Share any change that could help the next carer.'])
    : (health.length ? ['Read the linked observations before care starts.', 'Observe and record any change using factual language; contact the owner and veterinarian if concerned.'] : ['Follow the owner-confirmed food, movement and rest routine.', 'Add the next useful observation so the owner has a continuous record.']);
...
const views = {
  ...
  handoff() {
    const key=state.dog,d=dogs[key],summary=handoffSummary(key),audience=state.handoffAudience==='owner'?'Owner':'Next carer';
    setHeader('DAILY CARE HANDOFF', `${d.name} · ${audience}`);
    return `<div class="handoff-heading"><div><p class="eyebrow">CARE CONTINUITY · TODAY</p><h2>A clear handoff for whoever cares next</h2><p>Built from today's captured observations. Review before sharing or starting care.</p></div><div class="audience-toggle" aria-label="Handoff audience"><button class="${state.handoffAudience==='owner'?'active':''}" data-audience="owner">Owner</button><button class="${state.handoffAudience==='carer'?'active':''}" data-audience="carer">Next carer</button></div></div>
      <div class="dog-picker handoff-dogs">${Object.entries(dogs).map(([k,x])=>`<button class="dog-pick ${k===key?'active':''}" data-handoff-dog="${k}"><span>${x.emoji}</span><div><strong>${x.name}</strong><small>${todayObservations(k).length} observations today</small></div></button>`).join('')}</div>
      <div class="grid handoff-grid"><section class="card handoff-main"><div class="handoff-card-head">${dogAvatar(key)}<div><span class="handoff-label">Prepared for ${audience}</span><h2>${d.name}'s care highlights</h2><p>${summary.observations.length} captured moments · last update ${summary.observations[0]?.time || 'not yet recorded'}</p></div></div>
        <ol class="highlight-list">${summary.highlights.length?summary.highlights.map(item=>`<li>${escapeHtml(item)}</li>`).join(''):'<li>No observations captured today yet. Add a care moment before handoff.</li>'}</ol>
        <div class="health-note ${summary.health.length?'watch':''}"><strong>${summary.health.length?'Health observation — factual, not diagnostic':'Health context'}</strong><p>${summary.healthText}</p></div>
      </section><aside class="card next-care"><span class="handoff-label">NEXT CARE</span><h2>What to do next</h2><ul>${summary.guidance.map(item=>`<li>${item}</li>`).join('')}</ul><button class="primary" data-go="capture">＋ Add follow-up observation</button></aside></div>
      <section class="card evidence-panel"><div class="section-head"><div><h2>Evidence for this handoff</h2><p>Open any source observation in ${d.name.split(' ')[0]}'s care timeline.</p></div><div><button class="link-button" data-go="story">Review owner story</button><button class="ghost" data-go="gallery">View today's gallery</button></div></div><div class="evidence-list">${summary.observations.length?summary.observations.map(o=>`<button class="evidence-row" data-evidence-dog="${key}" data-evidence-id="${o.id}"><time>${o.time}</time><span><strong>${escapeHtml(o.title)}</strong><small>${escapeHtml(o.text)}</small></span><b>View source →</b></button>`).join(''):'<p class="empty-evidence">No source observations available yet.</p>'}</div></section>`;
  },
